...
WHERE (SELECT COUNT(*) FROM helykapcs AS hkap1 WHERE hkap1.allovizid = alloviz.id) = 1

AND (SELECT COUNT(*) FROM helykapcs AS hkap2 WHERE hkap2.gpsid = telepulesgps.id) = 1;
